...
昨天我们只做了 Function Calling 的前半段——模型告诉我们"我想调 get_weather，参数是北京"。但问题来了：然后呢？

我一开始天真地以为模型会"自动"执行工具。毕竟它都知道该调哪个函数了，直接跑一下不就行了？
...
这就是我花了好一会儿才真正理解的事：**Function Calling 不是一次请求就完事，它是你的代码和模型之间的一个多轮对话。**
...
是不是很眼熟？没错，这就是我们上周在 D6 写的那个 Agent Loop。
...
走通流程的过程中，我踩了三个坑。每个坑都不大，但都让我卡了好一会儿。记下来，希望你能少走弯路。
...
这个坑更隐蔽。我的 `get_weather` 函数一开始返回的是一个 dict：
...
结果 API 报了个序列化错误。原因很简单：**tool message 的 `content` 字段必须是字符串。**
...
搞定了单工具的完整流程之后，自然就会遇到一个问题：**当你有好几个工具时，代码怎么组织？**
...
工具一多就受不了了。后来我发现大家都用一个叫"注册表"的模式：
