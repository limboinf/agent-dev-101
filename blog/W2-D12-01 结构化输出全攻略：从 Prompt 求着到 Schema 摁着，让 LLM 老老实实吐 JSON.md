...
> 💡 **预告**：明天 D13 是 W2 周五项目日，把这周学的 Function Calling + ReAct + 结构化输出全部整合到一个 **真的能在终端里跑的 CLI Agent**——能查维基百科、能算数学、能查中国大陆天气，思考 / 工具调用 / 工具返回 / 最终回答用 Rich 的不同颜色和面板区分得清清楚楚。这就是从"看会"到"做会"的最后一公里——也是下周开始啃 LangChain 框架前必须先打通的内功。
