...
|  3  | 为什么 Agent 需要 Context Engineering？ | LLM 注意力有限 — "Lost in the Middle" 问题；上下文窗口越大不等于越好，信号被噪声稀释（Context Rot）；Agent 多步循环中每一步都需要不同的最优上下文；核心目标：在每一步发送最小、最高信号的 Token 集合                                                                                                         | 理解了"为什么" → 学习核心模式                                             |
...
| # | 知识点 | 要点 | 与下一知识点的关联 |
|:-:|--------|------|--------------------|
| 1 | 什么是 AI Agent？ | Agent 的定义：能自主感知环境、做出决策并采取行动的系统；Agent ≠ Chatbot 的核心区别 | 理解定义后 → 需要知道 Agent 内部是怎么运转的 |
| 2 | 感知-推理-行动循环 | Agent 的核心循环：Perceive → Reason → Act；每一步对应什么操作；循环何时终止 | 这个循环有不同的实现模式 → |
| 3 | Workflow vs Agent | Anthropic 的核心观点：大部分场景用 Workflow 就够了；何时需要真正的 Agent；过度设计的代价 | 理解"不是所有问题都需要 Agent" → |
...
|  #  | 知识点                               | 要点                                                                              | 与下一知识点的关联                          |
| :-: | --------------------------------- | ------------------------------------------------------------------------------- | ---------------------------------- |
|  1  | ReAct 论文核心思想                      | Reasoning + Acting 的统一；与纯 CoT（只想不做）、Act-only（只做不想）的对比                           | 理解理论 → 看具体执行流程                     |
|  2  | Thought → Action → Observation 循环 | 每一步的含义：Thought（模型的推理过程）→ Action（选择工具+参数）→ Observation（工具执行结果）；循环终止条件            | 理解循环后 → 如何用代码实现？                   |

**实践任务**: 阅读 ReAct 论文核心部分，画出 Thought-Action-Observation 循环流程图。

---

### Day 11 · ReAct 模式深入（下）

**主线**: 从零实现 ReAct Agent，理解 Prompt 设计挑战

|  #  | 知识点                               | 要点                                                                              | 与下一知识点的关联                          |
| :-: | --------------------------------- | ------------------------------------------------------------------------------- | ---------------------------------- |
|  1  | 从零实现 ReAct Agent                  | 不使用任何框架，用 Python + OpenAI API 实现：Prompt 模板设计、循环控制、工具分发、结果注入                     | 实现后 → 发现 Prompt 设计的挑战              |
|  2  | ReAct Prompt 模板设计                 | 如何在 System Prompt 中定义 ReAct 格式；如何引导模型输出规范的 Thought/Action/Observation；常见格式问题与修复 | Prompt 控制输出格式 → 需要更可靠的结构化输出 |
|  3  | ReAct 的局限与改进                      | 循环可能陷入死循环；模型可能格式错误；Token 消耗大；改进方向（Planning、Reflection）                          | 这些局限将在框架中被系统性解决                 |
...
**主线**: 让模型的输出可靠、可解析 — 从自由文本到结构化数据

|  #  | 知识点                             | 要点                                                                     | 与下一知识点的关联                          |
| :-: | ------------------------------- | ---------------------------------------------------------------------- | ---------------------------------- |
|  1  | 为什么需要结构化输出？                     | Agent 需要程序化处理模型输出；自由文本难以解析；与下游系统集成需要固定格式                               | 知道原因 → 学习实现方式                      |
|  2  | JSON Mode                       | OpenAI 的 response_format: json_object；如何在 Prompt 中配合描述 JSON 结构；常见陷阱    | JSON Mode 只保证"是 JSON" → 无法保证"结构正确" |
|  3  | Structured Output (JSON Schema) | 通过 JSON Schema 严格约束输出结构；OpenAI 的 Structured Outputs 功能；Anthropic 的实现差异 | Schema 定义了结构 → 需要在代码中验证            |

**实践任务**: 使用 JSON Mode 和 Structured Output 分别实现同一个任务，对比差异。

---

### Day 13 · 结构化输出（下）

**主线**: Pydantic 验证与 Agent 中的结构化输出实战

|  #  | 知识点                             | 要点                                                                     | 与下一知识点的关联                          |
| :-: | ------------------------------- | ---------------------------------------------------------------------- | ---------------------------------- |
|  1  | Pydantic 模型验证                   | 用 Pydantic 定义数据模型；自动验证 + 类型转换；与 LLM 输出的结合使用                            | Pydantic 是 Python 生态中结构化数据的标准 →    |
|  2  | 在 Agent 中使用结构化输出                | 工具参数的结构化；Agent 最终输出的结构化；错误重试策略（模型输出不合格时自动重试）                           | 结构化输出让 Agent 更可靠    |
|  3  | 面试题：怎么稳定输出 JSON                  | 各种稳定输出 JSON 的技巧与方案对比                                                                     | 实战必备知识                                 |

**实践任务**: 让 Agent 输出结构化的旅行计划 JSON（包含目的地、日程、预算等字段），使用 Pydantic 模型进行验证。
